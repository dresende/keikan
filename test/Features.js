...
describe("Features", () => {
	it("<% include %> can be used to load another view", async () => {
		...
		(await view({ name: "world" })).should.equal("<h3>\n\tHello world\n</h3>");
	});
	...
	it("<% include %> can have another default extension", async () => {
		...
		(await view()).should.equal("<h3>Hello world</h3>");
	});
	...
	it("<% include %> can be called with no arguments", async () => {
		...
		(await view1()).should.equal("Hello undefined");
		(await view2()).should.equal("Hello undefined");
	});
	...
	it("keeps empty lines", async () => {
		...
		(await view()).should.equal("<b>this is\n\nan example</b>");
	});
